...
public final class RegisterCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 2) {
            LanguageManager.send(player, "auth.register.usage",
                    "{prefix}&7Usage: &b/" + label.toLowerCase(Locale.ROOT) + " <password> <confirm>",
                    "prefix", manager.getPrefix(player),
                    "command", label.toLowerCase(Locale.ROOT));
            return true;
        }
        manager.attemptRegister(player, args[0], args[1]);
        return true;
    }
